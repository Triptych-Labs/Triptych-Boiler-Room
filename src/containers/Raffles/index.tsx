...
const RafflesFeatured = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTicks((ticks) => {
        if (Object.keys(quests).length > 0) {
          setQuest(Object.keys(quests)[ticks % Object.keys(quests).length]);
        }
  ...
  return (
    <div>
      <Typography sx={{color: "#cadf8c"}} align="left" gutterBottom variant="h5" component="div">
        {
          //@ts-ignore
          String(
            "Featured Raffles!"
      ...
      <div
        className="raffles-featured-box"
        style={{
          borderRadius: '25px',
      ...
      >
        {quests.hasOwnProperty(quest) &&
          <AnimatePresence exitBeforeEnter>
            <motion.div
              initial={{opacity: 0}}
              animate={{opacity: 0.8, transition: {duration: 2.5}}}
            ...
            >
              <div style={{margin: "20px"}}>
                <StyledCard className="raffles-featured-card">
                  <CardMedia
                    style={{height: "280px"}}
                    component="img"
                    height="280"
...
//@ts-ignore
export const RafflesGalleryItems = ({onSelection, onManage, onReward}) => {
  const [quests] = useRecoilState(questsAtom);
            ...
            <Grid xs={4} key={quest}>
              <Box textAlign="center">
                <StyledCard>
                  ...
                  <CardActions style={{justifyContent: "center"}}>
                    <Link href={String("/raffles/" + quest)}>
                      <Button
                        style={{
                          display: "flex",
                          justifyContent: "center",
...
export const RafflesGallery = () => {
  ...
  const [quests, setRaffles] = useRecoilState(questsAtom);
  const [, setQuested] = useRecoilState(questedAtom);
  const [questSelection, setRafflesSelection] =
  ...
  const [questsProgression, setRafflesProgression] = useRecoilState(
    questsProgressionAtom
  );
  ...
  const onBack = useCallback(
    (_) => {
      if (questsProgression > 0) setRafflesProgression(questsProgression - 1);
      if (questsProgression < 0) setRafflesProgression(questsProgression + 1);
    },
    [questsProgression, setRafflesProgression]
  );
  ...
  const onManage = useCallback(
    (_, quest) => {
      ...
      setRafflesSelection(quest);
      setRafflesProgression(-1);
    },
    [
      resync,
      setRafflesProgression,
  ...
  const onQuestStart = useCallback(
    (_, quest) => {
      async function startRaffles() {
        const startRafflesIx = JSON.parse(
          String.fromCharCode(
            ...(await start_quests(
              // @ts-ignore
              wallet.publicKey.toString(),
        ...
        if (Object.keys(startRafflesIx).length > 0) {
          const startRafflesTx = Transaction.populate(
            new Message(startRafflesIx.message)
          );
          startRafflesTx.recentBlockhash = (
            await connection.getRecentBlockhash("finalized")
          ).blockhash;
          const signature = await wallet.sendTransaction(
            startRafflesTx,
          ...
          console.log(signature);
          await connection.confirmTransaction(signature, "confirmed");
          setRafflesProgression(0);
        }
      }

      startRaffles();
    },
    [connection, quests, nfts, nftsSelection, wallet, setRafflesProgression]
  );
  ...
  const onQuestAction = useCallback(
    (_, quest) => {
      ...
      async function endRaffles() {
        const endRafflesIx = JSON.parse(
          String.fromCharCode(
            ...(await end_quests(
              // @ts-ignore
              wallet.publicKey.toString(),
        ...
        if (Object.keys(endRafflesIx).length > 0) {
          const endRafflesTx = Transaction.populate(
            new Message(endRafflesIx.message)
          );
          endRafflesTx.recentBlockhash = (
            await connection.getRecentBlockhash("finalized")
          ).blockhash;
          const signature = await wallet.sendTransaction(
            endRafflesTx,
